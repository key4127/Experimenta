...
from experimenta.agent.deepwiki.config import configs, DEFAULT_EXCLUDED_DIRS, DEFAULT_EXCLUDED_FILES

from urllib.parse import urlparse, urlunparse, quote
import requests
from requests.exceptions import RequestException
# from api.ollama_patch import OllamaDocumentProcessor
from experimenta.agent.tool.embedder import get_embedder
...
def count_tokens(text: str, embedder_type: str = None, is_ollama_embedder: bool = None) -> int:
    ...
    try:
        ...
        if embedder_type is None:
            from experimenta.agent.deepwiki.config import get_embedder_type
...
def prepare_data_pipeline(embedder_type: str = None, is_ollama_embedder: bool = None):
    ...
    """
    from experimenta.agent.deepwiki.config import get_embedder_config, get_embedder_type
...
class DatabaseManager:
    ...
    def cleanup_repo(self) -> None:
        """
        Delete the cloned repository directory after content has been generated.

        - Only applies to remote repositories (http/https).
        - Never deletes a user-provided local path.
        - Keeps the generated database file so embeddings can be reused.
        """
        try:
            ...
            if not save_repo_dir:
                return

            # Do not delete if the original input was a local path
            if self.repo_url_or_path and not self.repo_url_or_path.startswith(("http://", "https://")):
                logger.info(f"Skipping repo cleanup for local path: {save_repo_dir}")
                return

            if os.path.exists(save_repo_dir):
                shutil.rmtree(save_repo_dir)
                logger.info(f"Deleted cloned repository directory: {save_repo_dir}")
        except Exception as e:
            logger.error(f"Failed to clean up repository directory: {e}")
