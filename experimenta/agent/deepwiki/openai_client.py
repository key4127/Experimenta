...
class OpenAIClient(ModelClient):
    ...
    def init_sync_client(self):
        """
        初始化同步客户端。
        优先读取自身配置的 env_api_key_name；
        如果是默认的 OPENAI_API_KEY 且未设置，则兼容性兜底使用 EMBEDDING_API_KEY。
        """
        api_key = self._api_key or os.getenv(self._env_api_key_name)
        if not api_key and self._env_api_key_name == "OPENAI_API_KEY":
            # 兼容历史：老的 embedder / 旧数据库里序列化的 OpenAIClient 只认 OPENAI_API_KEY，
            # 但现在我们允许用户只配置 EMBEDDING_API_KEY 来专门给 embedding 用。
