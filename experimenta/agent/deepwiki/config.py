...
def load_generator_config():
    ...
    if "providers" in generator_config:
        for provider_id, provider_config in generator_config["providers"].items():
            ...
            # Fall back to default mapping based on provider_id (no Google by design)
            elif provider_id in ["openai", "ollama", "deepseek"]:
                default_map = {
                    "openai": OpenAIClient,
...
def get_model_config(provider="deepseek", model=None):
    ...
    Parameters:
        provider (str): Model provider ('openai', 'ollama', 'deepseek')
    ...
    # 兼容旧调用：如果外部还传入 'google'，自动映射到 deepseek
    if provider == "google":
        logger.warning("Provider 'google' is no longer supported, falling back to 'deepseek'.")
        provider = "deepseek"
